...
- [OK] Remover bot telegram/whatsapp e alterar a página de Integração Bot para um assistente de IA interno onde pode tirar dúvidas sobre os dados do usuário, ajuda a planejar orçamentos, traçar metas, etc. Esse assistente deve estar disponível para o mobile também na barra de menu inferior. O recurso será somente para planos pro e familia
  ...
  >
  > Implementado: bot removido por completo (rotas `/api/bots/*`, workers de mensagens e de transcrição, dependência `grammy`, models `BotConversation`/`BotMessage` e as colunas de Telegram/WhatsApp em `user_profiles`). No lugar entrou `/api/assistant` (`apps/api/src/routes/assistant.ts`): conversas persistidas com histórico multi-turno, laço de tool calling sobre as 5 ferramentas financeiras extraídas para `src/lib/ai/finance-tools.ts` (reaproveitadas também pelo `POST /api/ai/query`), gate de plano `isAssistantAllowed` (Pro/Família) e contabilização de consumo com `feature: "assistant"`.
  >
  > Agentes personalizados com seleção de ferramentas por agente (`AssistantAgent.enabledTools`): nenhuma marcada = acesso a tudo; marcando algumas, o limite é declarado no system prompt e o agente recusa perguntas fora do escopo em vez de estimar um valor a partir do que tem. Quatro modelos prontos em `src/lib/ai/agent-presets.ts` (Coach de economia, Analista de orçamento, Organizador de contas, Planejador de metas) abrem o formulário já preenchido pro usuário ajustar antes de salvar.
  >
  > Interfaces: `/assistant` na web (lista de conversas + chat + gerenciador de agentes), aba "IA" na barra inferior do mobile (`app/(tabs)/assistant.tsx`) mais a tela de agentes, e em `/admin/ai` o campo "Modelo do assistente" (`assistantModel`) com o kill-switch `assistantEnabled`.
  >
  > Pendência conhecida: a tela do mobile não lista conversas anteriores — cada abertura inicia uma nova, e o histórico só é navegável pela web.

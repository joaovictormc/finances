...
| Zero testes automatizados | 11 arquivos de teste, 85 casos: `pix` (CRC-16/CCITT-FALSE contra vetor publicado + estrutura EMV), `payment-methods` (mascaramento e criptografia em repouso dos segredos), `plan-limits` (gates de plano e expiração de assinatura), `subscription-period` (avanço do vencimento e soma de meses sem transbordo), `plan-grant` (concessão de dias de plano e suas bordas), `prize-description`, `notifications` (canal in-app x e-mail), `pix-checkout` (janela de confirmação e varredura de pendentes), `import-limits`, `bulk-categorize`, `report-period`. |
| Renovação da assinatura não avançava o período | O webhook só tratava `subscription_preapproval`. O Mercado Pago não reemite esse evento a cada cobrança — o preapproval segue `authorized` e o que chega é `subscription_authorized_payment`, que não era tratado em lugar nenhum. `currentPeriodEnd` era gravado uma vez na autorização e nunca mais avançava, então com a checagem de expiração quem continuava pagando perdia o plano em ~33 dias. Agora renova só com pagamento aprovado, somando sobre o que resta ou a partir de hoje se já venceu. **Exige o evento marcado no painel do Mercado Pago** (Webhooks → *Pagamentos de assinatura*). |
| Preço dos planos fixo em código | Era `apps/api/src/lib/plans.ts`, e reajustar exigia deploy. Virou a tabela `PlanPrice` (plano × período), editável em `/admin/pricing`, com equivalente mensal e desconto calculados na tela. As features do plano continuam em código — só o preço virou dado. Preço novo vale só pra checkouts novos: o valor de um preapproval é fixado na criação, então quem já assina continua no que contratou. |
| Só existia plano mensal | `Subscription.interval` (monthly/semiannual/annual) alimenta a recorrência do preapproval, o tamanho do ciclo na confirmação manual do Pix — que antes liberava sempre 30 dias, mesmo pra quem pagou um ano — e o preço cobrado. Seletor de período na tela de planos, web e mobile. |
| Prêmio da roleta prometia o que não entregava | O prêmio só tinha rótulo e pontos, e o rótulo é texto livre: "1 ano de Plano Família" com `points: 100` dava confete e 100 pontos. `SpinPrize.type` passou a decidir a entrega — `plan_days` soma dias de assinatura na hora (`plan-grant.ts`), sem fila de resgate. Nunca rebaixa quem já tem plano melhor, e não data acesso sem prazo. |
| Recompensa de indicação somava sobre data vencida | `currentPeriodEnd ?? new Date()` errava nas duas pontas: período vencido somava sobre uma data no passado (bônus sem valor), e período nulo — acesso sem prazo do admin — virava assinatura de 30 dias, rebaixando quem indicou. Passou a usar a mesma `decidePlanGrant` do prêmio da roleta. |
| App crashava ao abrir pelo Expo Go | `expo-notifications` e `expo-blob` resolvem o módulo nativo já no `import` (`requireNativeModule` em escopo de módulo, fora de qualquer `try`), e o `_layout` carregava os dois no boot. Passaram a ser carregados sob demanda e protegidos. Push continua exigindo development build — a Expo o removeu do Expo Go no SDK 53. |
| Pontos parados na visão geral do mobile | `PointsBadge` e `GamificationCard` buscavam o perfil em `useEffect` com deps vazias, só na montagem; a aba fica montada, então gastar pontos em `/rewards` e voltar não atualizava nada. Passaram a usar `useFocusEffect`, como o resto da tela já fazia. |
...
| Item | Detalhe |
|---|---|
| **Testes só de unidade** | Os 85 casos cobrem funções puras e quatro módulos com banco mockado. Nenhuma rota Hono é exercida ponta a ponta, e os workers de fila (`jobs/workers/*`) não têm teste nenhum. O próximo degrau natural é um teste de rota com o app Hono em memória. |

...
| Item | Detalhe |
|---|---|
| **Testes só de unidade** | Os 59 casos cobrem funções puras e quatro módulos com banco mockado. Nenhuma rota Hono é exercida ponta a ponta, e os workers de fila (`jobs/workers/*`) não têm teste nenhum. O próximo degrau natural é um teste de rota com o app Hono em memória. |
| **Campainha de notificações só na web** | `apps/mobile` não tem equivalente: a API (`/api/notifications`) já serve os dois, falta a UI. Usuário que só usa o celular continua dependendo do e-mail. |
| **Notificação sem push de verdade** | A campainha consulta a cada 60s e ao voltar pra aba; não há WebSocket nem push do navegador, então um aviso pode demorar até um minuto pra aparecer. Suficiente pro que existe hoje (avisos diários), insuficiente se algum dia houver aviso que precise chegar na hora. |
